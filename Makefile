# $Id: Makefile,v 1.79 2025/10/23 14:04:36 albert Exp $
...
TESTTARGETS= test.bin               \
lina530.asm lina405.asm rf751.asm   \
lina530   lina405   rf751           \
rf751.cul
...
ciasdissrczip : $(RELEASECIASDIS) ; echo ciasdis-dev-$(VERSION).tar.gz $+ | xargs tar -cvzf
...
testasriscv: asgen.frt asriscv.frt testsetriscv ; \
	rm -f $@.diff ;\
	echo CR  \"INCLUDE\" WANTED  \"DUMP\" WANTED INCLUDE asgen.frt INCLUDE asriscv.frt INCLUDE testsetriscv |\
	$(FORTH) -a  |\
	sed '1,/TEST STARTS HERE/d' |\
	sed 's/^[0-9A-F \.,]*://' >$@ ;\
	diff -w $@ testsetriscv >$@.diff ;\
	$(DIFF_TXT) $@.diff | true
...
testasses : testasalpha testasriscv testas6809 testas80 testas86 testallpentium
...
gsetall : gsetalpha gset6809 gset80 gset86 gset386-16 \
	  gset386 gsetpentium gsetAMD gsetriscv
...
# This can be checked against expectation. Once that is done
# it can be used as a testsetxxx.
gsetAMD: asgen.frt asi386.frt ; \
    (echo CR; cat $+;echo ASSEMBLER HEX BITS-64   SHOW-ALL)|\
    $(FORTH) -a|\
    sed 's/~SIB|   10 SIB,,/[DX +1* DX]/' |\
    sed 's/~SIB|   18 SIB,,/[DX +1* BX]/' |\
    sed 's/~SIB|   1C SIB,,/[AX +1* 0]/' |\
...
gsetriscv: asgen.frt asriscv.frt ; \
    echo CR \"DUMP\" WANTED INCLUDE asgen.frt INCLUDE asriscv.frt ASSEMBLER HEX SHOW-ALL |\
    $(FORTH) -a >$@       ; \
    $(DIFF_TXT) $@
...
# This uses an auxiliary file ..equ.. generated by crawling and saved.
lina405.asm : ciasdis lina405equ.cul lina405.cul testcmp/lina405
	ciasdis -d testcmp/lina405 lina405.cul >$@
	ciasdis -a $@ lina405
	$(DIFF_BIN) lina405
	$(DIFF_TXT) $@
...
rf751.cul : ciasdis rf751equ.cul rfcrawl.cul elf.cul testcmp/rf751
	echo FETCH testcmp/rf751 INCLUDE rfcrawl.cul | ciasdis >$@
	$(DIFF_TXT) $@
...
# This only uses a source generated by crawling.
lina530.asm : ciasdis lina530crawl.cul testcmp/lina530
	ciasdis -d testcmp/lina530 lina530crawl.cul 2>&1 >$@
	ciasdis -a $@ lina530
	$(DIFF_BIN) lina530
	$(DIFF_TXT) $@
...
testexamples64 :  test.bin lina405.asm rf751-64.asm lina530.asm

# -----------------
regressiontest64 : testasses testexamples64 gsetall gset386.diff
regressiontest32: testasses testexamples gsetall gset386.diff
